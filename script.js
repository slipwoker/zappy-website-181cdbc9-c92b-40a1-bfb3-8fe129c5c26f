document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('click', function (e) {
    ...
    const offsetPosition = elementTop - navbarHeight - 16;
  ...
  const messages = {
    ...
    maxLength: isRTL ? 'يجب ألا يتجاوز هذا الحقل {max} حرفاً' : 'This field must not exceed {max} characters',
  ...
  function validateField(field) {
    ...
    if (value.trim()) {
      ...
      if ((type === 'tel' || name.toLowerCase().includes('phone') || name.toLowerCase().includes('tel')) && !validatePhone(value)) {
        showFieldError(field, messages.phone);
        return false;
      }

      if (minLength && value.trim().length < parseInt(minLength)) {
        showFieldError(field, messages.minLength.replace('{min}', minLength));
        return false;
      }

      if (maxLength && value.trim().length > parseInt(maxLength)) {
        showFieldError(field, messages.maxLength.replace('{max}', maxLength));
        return false;
      }
    }
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    if (isFormValid) {
      ...
      if (submitBtn) {
        ...
        submitBtn.textContent = isRTL ? 'جارٍ الإرسال...' : 'Sending...';
      }
  ...
  if (animatableElements.length > 0) {
    animatableElements.forEach(function (el, index) {
      ...
      const delay = el.getAttribute('data-delay') || (index % 5) * 100;
    ...
    const scrollObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          ...
          entry.target.classList.remove('scroll-animation-ready');
  ...
  // ============================================================
  // UTILITY: Inject base animation styles if not present
  // ============================================================
  if (!document.getElementById('scroll-animation-styles')) {
    const styleEl = document.createElement('style');
    styleEl.id = 'scroll-animation-styles';
    styleEl.textContent = [
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "שי פורטמן דפוס ושילוט משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '181cdbc9-c92b-40a1-bfb3-8fe129c5c26f',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '181cdbc9-c92b-40a1-bfb3-8fe129c5c26f';
            ...
            if (mobileToggle && navMenu) {
                ...
                if (phoneHeaderBtn) {
                    phoneHeaderBtn.addEventListener('click', function() {
                        ...
                        // Falls back to 0507769693 placeholder which businessInfoUpdater can replace
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '181cdbc9-c92b-40a1-bfb3-8fe129c5c26f';
